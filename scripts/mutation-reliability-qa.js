...
assert.match(html, /id="pri-btn"[^>]*type="button"[^>]*style="display:none;"/, 'the shared top-bar action must start hidden so the initial Dashboard does not show a duplicate New Job button');
assert.match(html, /if\(id==='dashboard'\)\{pri\.style\.display='none';\}/, 'Dashboard navigation must keep the shared top-bar action hidden');
assert.match(html, /else if\(id==='jobs'\)\{pri\.textContent='\+ New Job';\}/, 'Jobs navigation must restore the shared New Job action');
assert.match(html, /Some dashboard totals could not be refreshed\. Jobs and other saved information are still available\./, 'Dashboard summary failure must retain local actions and saved state');
...
assert.match(html, /api\('GET','\/api\/shop-context'/, 'frontend must call the safe shop-context endpoint for shop identity and membership display');
assert.match(html, /await loadShopContextDetails\(\);\s*\n\s*const failures=await loadAll/, 'startup must validate shop session before bulk API fanout');
assert.match(html, /await loadShopContextDetails\(\);\s*\n\s*await loadAll/, 'focus refresh must validate shop session before bulk API fanout');
assert.match(html, /shopContextDetails\.membership/, 'sidebar shop label should use server-confirmed membership details, not raw stored identity');
...
assert.match(html, /<label>Price \(\$\)<\/label><input type="number" id="catf-price"/, 'Service Catalog form must label service price without Default');
...
assert.match(html, /function defaultJobItemRate\(type\)/, 'repair-order lines must share configured labor and diagnostic defaults');
const addJobItemStart = html.indexOf('function isLaborLineItem(type)');
const addJobItemEnd = html.indexOf('\nfunction addJobItemFromCatalog()', addJobItemStart);
assert.ok(addJobItemStart >= 0 && addJobItemEnd > addJobItemStart, 'job item defaults and handler source must be extractable');
const jobItemContext = { document:{getElementById(){return{value:''};}}, state: { customers:[], settings: { default_labor_rate: 125, diagnostic_rate: 150, fleet_rate: 100, emergency_rate: 175 } } };
...
assert.match(html, /repairOrderInput\.value=nextRepairOrderNumber\(\)/, 'new jobs must receive the next suggested repair-order number');
assert.match(html, /repairOrderInput\.dataset\.suggested=repairOrderInput\.value/, 'new jobs must remember which RO number is only a client suggestion');
assert.match(html, /serverAssignsRepairOrder=!editJobId&&\(!repairOrderNumber\|\|repairOrderNumber===repairOrderInput\.dataset\.suggested\)/, 'untouched RO suggestions must be assigned authoritatively by the server');
assert.match(html, /customerSelect\.disabled=!!id/, 'job editing must make the immutable customer relationship explicit');
assert.match(html, /vehicleSelect\.disabled=!!id/, 'job editing must make the immutable vehicle relationship explicit');
assert.match(html, /ensureLinkedSelectOption\(customerSelect,j\.customer_id/, 'archived customer links must remain visible on historical repair orders');
assert.match(html, /ensureLinkedSelectOption\(vehicleSelect,j\.vehicle_id/, 'archived vehicle links must remain visible on historical repair orders');
assert.match(html, /Archived-customer repair orders are retained for history\./, 'historical repair orders must not offer a delete action that the API will reject');
assert.match(html, /state\.customers\.find\(x=>x\.id===j\.customer_id\)\|\|\{first:j\.first,last:j\.last,phone:j\.customer_phone/, 'invoices must retain archived customer details from the repair-order record');
assert.match(html, /jobItems = \[\];\s*await loadVehiclesForJob\(\)/, 'new-job initialization must clear stale vehicle options from a previous edit');
assert.match(html, /if\(!editJobId&&!vehs\.length\)toast\('This customer has no vehicles\./, 'new-job form must guide users when the selected customer has no vehicle');
assert.match(html, /addrEl\.value===previousAuto/, 'customer changes must replace only an automatically populated service address');
assert.match(html, /applyInventoryQuantityUpdates\(inventoryUpdates\)/, 'job saves must apply server-confirmed inventory quantities without a broad refresh');
...
const lineItemTypeContext = { document:{getElementById(){return{value:''};}}, state:{customers:[],settings:{default_labor_rate:125,diagnostic_rate:150,fleet_rate:100,emergency_rate:175}}, jobItems:[{ type:'labor', qty:2.5, rate:125, taxable:0 }], renderJobItems(){} };
...
assert.strictEqual(lineItemTypeContext.qa.isLaborLineItem('emergency'), true, 'emergency work must use the labor-hours column');
assert.strictEqual(lineItemTypeContext.qa.isLaborLineItem('part'), false, 'parts must use the quantity column');
lineItemTypeContext.qa.setJobItemType(0,'diagnostic');
assert.strictEqual(lineItemTypeContext.jobItems[0].rate, 150, 'switching an untouched labor line to diagnostic must use the configured diagnostic rate');
...
lineItemTypeContext.jobItems[0].inventory_id=7;
lineItemTypeContext.qa.setJobItemType(0,'labor');
assert.strictEqual(lineItemTypeContext.jobItems[0].inventory_id, null, 'switching an inventory part to a non-part charge must clear its inventory link');
assert.match(html, /const TYPES = \['labor','diagnostic','emergency','part','parts','shop_supply','fee','sublet','discount'\]/, 'job editing must represent every supported estimate and repair-order line type');
assert.match(html, /function closeTopmostModal\(\)[\s\S]*?querySelectorAll\('\.modal-overlay\.open'\)[\s\S]*?openModals\.at\(-1\)/, 'Escape handling must close only the topmost open modal');
assert.match(html, /if\(e\.key==='Escape'\)[\s\S]*?else if\(closeTopmostModal\(\)\)e\.preventDefault\(\)/, 'the global keyboard handler must close an open modal with Escape');
...
assert.match(estimatesRoute, /date, miles, labor, labor_hours, labor_rate, parts, discount, tax_rate, status/, 'estimate conversion must carry mileage, labor detail, discount, and its saved tax rate into the repair order');
...
assert.match(jobsRoute, /const shouldRecordPayment = req\.body\.invoice_status === 'Paid'/, 'an explicit Paid repair-order save must reconcile a newly introduced balance');
