...
assert.match(main, /ipcMain\.on\('app:get-version'[\s\S]*app\.getVersion\(\)/, 'sandbox-safe application-version IPC is missing');
...
assert.doesNotMatch(preload, /require\(['"]\.\.\/package\.json['"]\)/, 'sandboxed preload must not require package.json');
assert.match(preload, /ipcRenderer\.sendSync\('app:get-version'\)/, 'preload must obtain the version through the narrow IPC channel');
...
assert.match(html, /function browserPrintDocument[\s\S]*document\.createElement\('iframe'\)/, 'browser printing must not depend on a pop-up window');
assert.doesNotMatch(html, /window\.open\('','_blank','width=800,height:900'\)/, 'invoice printing must not use a blockable pop-up');
assert.match(html, /browserPrintDocument\(\{saveAsPdf:true\}\)/, 'browser PDF fallback must open the print dialog with Save as PDF guidance');
